document.addEventListener("DOMContentLoaded", function () {
  ...
  let tableDepartamento = $("#tableDepartamento").DataTable({
    aProcessing: true,
    aServerSide: true,
    responsive: true,
    language: {
    ...
    ajax: {
      url: base_url + "/Listado/getDepartamentos",
    },
    columns: [
      {
        data: null,
      ...
      { data: "estado" },
      {
        ...
        render: function (data, type, row) {
          ...
          if (permisosMod.editar == 1) {
            botones += `
                        <button type="button" class="btn btn-primary edit-btn" data-bs-toggle="modal" data-bs-target="#editModal" data-id="${row.id_departamento}">
          ...
          if (permisosMod.eliminar == 1) {
            botones += `
                        <button type="button" class="btn btn-danger delete-btn" data-id="${row.id_departamento}">
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      let ajaxUrl = base_url + "/Listado/setDepartamento";
      ...
      success: function (response) {
        if (response.status) {
          ...
          response.data.forEach(function (areas) {
            opciones += `<option value="${areas.id_area}">${areas.nombre_area} ${areas.descripcion}</option>`;
          });
  ...
  $(document).on("click", ".edit-btn", function () {
    ...
    cargarUsuarios("edit_area");

    $.ajax({
      url: `${base_url}/Listado/getDepartamentobyID/${idEmpresa}`,
      method: "GET",
      dataType: "json",
      success: function (response) {
        if (response.status) {
          $("#edit_id_departamento").val(response.data.id_departamento);
          $("#edit_nombre_departamento").val(response.data.nombre_departamento);
    ...
    if (result.isConfirmed) {
      try {
        const response = await $.ajax({
          url: `${base_url}/Listado/deleteDepartamento/${idEmpresa}`,
          method: "POST",
        });
        ...
        if (data.status) {
          Swal.fire({
            ...
            tableDepartamento.ajax.reload(); // Recarga solo la tabla
          });
        } else {
