document.addEventListener("DOMContentLoaded", function () {
  ...
  let tableDocumentos = $("#tableDocumentos").DataTable({
    aProcessing: true,
    aServerSide: true,
    responsive: true,
    language: {
    ...
    ajax: {
      url: base_url + "/Listado/getDocumentos",
    },
    columns: [
      {
        data: null,
      ...
      { data: "estado" },
      {
        ...
        render: function (data, type, row) {
          ...
          if (row.estado === "Activo") {
            actionButton = `<button type="button" class="btn btn-danger delete-btn" data-id="${row.id_documento}">
                  <i class="fas fa-trash-alt"></i>
              </button>`;
          } else {
            actionButton = `<button type="button" class="btn btn-warning habilitar-btn" data-id="${row.id_documento}">
                  <i class="fas fa-trash-alt"></i>
              </button>`;
          }
          ...
          if (permisosMod.editar == 1) {
            botones += `
              <button type="button" class="btn btn-primary edit-btn" data-bs-toggle="modal" data-bs-target="#editModal" data-id="${row.id_documento}">
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      let ajaxUrl = base_url + "/Listado/setDocumentos";
  ...
  $("#createModal").on("show.bs.modal", function () {
    ...
    $("#id_documento").val(""); // Asegúrate de que el campo oculto esté vacío
  ...
  $(document).on("click", ".edit-btn", function () {
    ...
    $.ajax({
      url: `${base_url}/Listado/getDocumentosbyId/${idEmpresa}`,
      method: "GET",
      dataType: "json",
      success: function (response) {
        if (response.status) {
          $("#edit_id_documento").val(response.data.id_documento);
    ...
    if (result.isConfirmed) {
      try {
        const response = await $.ajax({
          url: `${base_url}/Listado/deleteDocumentos/${idEmpresa}`,
          method: "POST",
        });
        ...
        if (data.status) {
          Swal.fire({
            ...
            tableDocumentos.ajax.reload(); // Recarga solo la tabla
          });
        } else {
  ...
  $(document).on("click", ".habilitar-btn", async function () {
    ...
    if (result.isConfirmed) {
      try {
        const response = await $.ajax({
          url: `${base_url}/Listado/habilitarDocumentos/${idEmpresa}`,
          method: "POST",
        });
        const data = JSON.parse(response);
        if (data.status) {
