...
try {
    ...
    if (!empty($arrData['jefes']['responsable_aprobacion'])) {
        $mail->addAddress($arrData['jefes']['correo_responsable']);
    } else {
        $mail->addAddress($arrData['jefes']['correo_aprobador_1']);
    }
    ...
    if (!empty($arrData['jefes']['responsable_aprobacion'])) {
        $jefe = "<p>Estimado/a <strong>{$arrData['jefes']['responsable_aprobador']}</strong>,</p>";
    } else {
        $jefe = "<p>Estimado/a <strong>{$arrData['jefes']['nombre_aprobador_1']}</strong>,</p>";
    }
    ...
    $mail->Body = "
        ...
        <body style='font-family: Arial, sans-serif; padding: 20px;'>
            <div style='max-width: 600px; padding: 20px; margin: auto; border-radius: 10px; box-shadow: 0px 0px 10px #ccc;'>
                ...
                $jefe
                <p>El usuario <strong>{$arrData['colaborador']['nombre_completo']}</strong>ah cancelado su solicitud de vacaciones con los siguientes detalles:</p>
                ...
                <table style='width: 75%; border-collapse: collapse; margin-top: 10px;'>
                    <tr>
                        <th style='border: 1px solid #ddd; padding: 8px; background:rgb(220, 36, 36); color: black;'>#</th>
                        <th style='border: 1px solid #ddd; padding: 8px; background:rgb(220, 36, 36); color: black;'>Fecha</th>
                        <th style='border: 1px solid #ddd; padding: 8px; background:rgb(220, 36, 36); color: black;'>Valor</th>
                        <th style='border: 1px solid #ddd; padding: 8px; background:rgb(220, 36, 36); color: black;'>Día</th>
                    </tr>
                    $detallesFechas
                </table>
