...
$confirmacion_correo_responsable = false;
...
try {
    ...
    $mail->addAddress($arrData['jefe']['correo_empresarial'], $arrData['jefe']['nombre_completo']); // Correo del jefe

    // Contenido dinámico
    $mail->isHTML(true);
    $mail->Subject = 'Nueva Solicitud de Vacaciones - ' . date('d/m/Y');

    // Construir cuerpo del correo con una tabla
    $detallesFechas = "";
    ...
    <body style='font-family: Arial, sans-serif; padding: 20px;'>
        <div style='max-width: 600px; padding: 20px; margin: auto; border-radius: 10px; box-shadow: 0px 0px 10px #ccc;'>
            ...
            <p>El usuario <strong>{$arrData['solicitud']['nombre_completo']}</strong>, ha realizado la siguiente solicitud de vacaciones:</p>
    ...
    // Enviar y confirmar
    $mail->send();
    $confirmacion_correo_responsable = true;

} catch (Exception $e) {
    error_log("Error en SolicitudEmailCC: " . $e->getMessage());
    $confirmacion_correo_responsable = false;
}
?>