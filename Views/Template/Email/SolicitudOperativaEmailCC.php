...
try {
    ...
    if ($arrData['estado'] == 'Pendiente Aprob. 2') {
        $revision = "<p>Se ha revisado la solicitud de <strong>{$arrData['solicitud']['nombre_completo']}</strong>  por parte de <strong>{$arrData['jefes']['nombre_aprobador_1']} </strong>:</p>";
    } elseif ($arrData['estado'] == 'Pendiente Aprob. 3') {
        $revision = "<p>Se ha revisado la solicitud de <strong>{$arrData['solicitud']['nombre_completo']}</strong>  por parte de <strong>{$arrData['jefes']['nombre_aprobador_2']}</strong>:</p>";
    }
    
    // Contenido dinámico
    $mail->isHTML(true);
    $mail->Subject = 'Seguimiento de Solicitud de Vacaciones';
    ...
    $mail->Body = "
        ...
        <body style='font-family: Arial, sans-serif; padding: 20px;'>
            <div style='max-width: 600px; padding: 20px; margin: auto; border-radius: 10px; box-shadow: 0px 0px 10px #ccc;'>
                <h2 style='color: #333; text-align: center;'>Seguimiento de Solicitud de Vacaciones</h2>
                $revision
                ...
                <table style='width: 35%; border-collapse: collapse; margin-top: 10px;'>
                    ...
                    <tr>
                        <td style='border: 1px solid #ddd; padding: 8px;'><strong>Seguimiento:</strong></td>
                        <td style='border: 1px solid #ddd; padding: 8px; color: black;'><strong>{$arrData['estado']}</strong></td>
                    </tr>
                    <tr>
                        <td style='border: 1px solid #ddd; padding: 8px;'><strong>Proxima Revision de :</strong></td>
                        <td style='border: 1px solid #ddd; padding: 8px;'>{$arrData['colaborador']['nombre_completo']}</td>
                    </tr>
                </table>
                ...
                <table style='width: 75%; border-collapse: collapse; margin-top: 10px;'>
                    <tr>
                        <th style='border: 1px solid #ddd; padding: 8px; background: blue; color: white;'>#</th>
                        <th style='border: 1px solid #ddd; padding: 8px; background: blue; color: white;'>Fecha</th>
                        <th style='border: 1px solid #ddd; padding: 8px; background: blue; color: white;'>Valor</th>
                        <th style='border: 1px solid #ddd; padding: 8px; background: blue; color: white;'>Día</th>
                    </tr>
                    $detallesFechas
                </table>
